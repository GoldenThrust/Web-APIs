...
function averagebatteryTime(arr) {
  ...
  const avg = (sum / arrLen).toFixed(1);
...
function drawStroke(strokeWidth, color, posPercent) {
  ...
  ctx.arc(
    halfCanvasWidth,
    halfCanvasHeight - calculatePercentage(canvasDepth, 2),
    sideStrokeRadius,
    0,
    Math.PI * 2
  );
...
function drawBattery() {
  ...
  let color;
  if (isCharging || batteryLevel > 70) color = "springgreen";
  else if (batteryLevel < 30) color = "red";
  else color = "hsl(" + 1 * batteryLevel + ", 100%, 50%)";
  ...
  ctx.arc(
    halfCanvasWidth,
    halfCanvasHeight - calculatePercentage(canvasDepth, 2),
    calculatePercentage(canvasDepth, 7),
    startAngle,
    endAngle
  );
  ...
  function animateCharging() {
    ctx.clearRect(
      halfCanvasWidth - calculatePercentage(canvasDepth, 1),
      halfCanvasHeight - calculatePercentage(canvasDepth, -0.5),
      calculatePercentage(canvasDepth, 1.5),
      calculatePercentage(canvasDepth, 3)
    );
    offset++;
    if (offset > 10) {
      offset = 0;
    ...
    ctx.moveTo(
      halfCanvasWidth,
      halfCanvasHeight + calculatePercentage(canvasDepth, 1)
    );
    ...
    ctx.shadowBlur = 4;
    ctx.lineTo(
      halfCanvasWidth + calculatePercentage(canvasDepth, -0.5),
      halfCanvasHeight + calculatePercentage(canvasDepth, 2)
    );
    ctx.lineTo(
      halfCanvasWidth + calculatePercentage(canvasDepth, 0.005),
      halfCanvasHeight + calculatePercentage(canvasDepth, 2)
    );
    ctx.lineTo(
      halfCanvasWidth + calculatePercentage(canvasDepth, -0.5),
      halfCanvasHeight + calculatePercentage(canvasDepth, 3)
    );
    ctx.stroke();
  ...
  ctx.fillText(
    estimatedTime,
    halfCanvasWidth - timeWidth / 2,
    halfCanvasHeight + calculatePercentage(canvasDepth, 8)
  );
  ctx.fill();
  ...
  ctx.fillStyle = theme;
  ctx.font = fontSize + "px cursive";
  const avgtimeWidth = ctx.measureText(
    "avg time - " + averagebatteryTime(batteryData.time) + "min"
  ).width;
  ctx.fillText(
    "avg time - " + averagebatteryTime(batteryData.time) + "min",
    halfCanvasWidth - avgtimeWidth / 2,
    halfCanvasHeight - calculatePercentage(canvasDepth, 5)
  );
  ctx.fill();
}
...
function drawBatteryUsageLineChart(estimatedTime) {
  ...
  for (
    var i = Math.min(...batteryData.time);
    i <= Math.max(...batteryData.time) && batteryData.time.length > 1;
    i++
  ) {
  ...
  ctx.fillText(
    "%",
    curX + 2 * cXrangeInc - canvasDepth / 400,
    cHeight + canvasDepth / 100
  );
  ...
  navigator.getBattery().then((battery) => {
    ...
    function updateChargingTimeInfo() {
      if (battery.chargingTime !== Infinity) {
        ...
        if (batteryData.level.includes(batteryLevel)) {
          batteryData.time[batteryData.level.indexOf(batteryLevel)] =
            parseInt(minInc);
        } else {
        ...
        batteryTime = "- cannot check battery charging time on your device";
    }
    ...
    function updateDischargingTimeInfo() {
      if (battery.dischargingTime !== Infinity) {
        ...
        batteryTime = "- cannot check battery discharging time on your device";
    }
  });
}
